...
export default function CompareRepositories({
    data,
}: WidgetContext<ComparisonData>) {
    if (!data) return <div className="p-4 text-red-500">No comparison data available.</div>;

    return (
        <div className="flex flex-col gap-4 p-4 font-sans text-sm w-full">
            <h2 className="text-xl font-bold text-center mb-4">Repository Comparison</h2>

            <div className="grid grid-cols-2 gap-6 w-full">
                {/* Repo 1 Column */}
                <div className="flex flex-col gap-4 border-r pr-4 dark:border-gray-700">
                    ...
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {data.repo1.data.tech_stack?.slice(0, 5).map((tech, i) => (
                                <span key={i} className="px-2 py-1 bg-gray-200 rounded text-xs dark:bg-gray-700">{tech}</span>
                            ))}
                        </div>
                    </div>

                    <div className="flex justify-between items-center bg-gray-50 p-3 rounded dark:bg-gray-800">
                        <span className="font-semibold text-xs">Complexity</span>
                ...
                <div className="flex flex-col gap-4 pl-2">
                    ...
                    <div>
                        <h4 className="font-semibold mb-2">Tech Stack</h4>
                        <div className="flex flex-wrap gap-2">
                            {data.repo2.data.tech_stack?.slice(0, 5).map((tech, i) => (
                                <span key={i} className="px-2 py-1 bg-gray-200 rounded text-xs dark:bg-gray-700">{tech}</span>
                            ))}
                        </div>
                    </div>
